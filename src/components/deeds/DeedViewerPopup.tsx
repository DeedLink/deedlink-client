...
import { FaFileSignature, FaUserShield, FaMapMarkedAlt, FaRoute, FaLayerGroup, FaExpand } from "react-icons/fa";
...
const DeedViewerPopup = ({ deed, onClose, currency = "USD", areaUnit = "m²" }: Props) => {
  ...
  const getLandTypeIcon = (type?: string) => {
    if (!type) return "❓";
    if (type.toLowerCase() === "paddy land" || type.toLowerCase() === "mada") return "🌾";
    if (type.toLowerCase() === "highland" || type.toLowerCase() === "kuburu" || type.toLowerCase() === "goda") return "🌲";
    return "❓";
  };

  const getLandTypeColor = (type?: string) => {
    if (!type) return "bg-gray-300 text-gray-800";
    if (type.toLowerCase() === "paddy" || type.toLowerCase() === "mada") return "bg-green-100 text-green-800";
    if (type.toLowerCase() === "highland" || type.toLowerCase() === "kuburu" || type.toLowerCase() === "goda") return "bg-yellow-100 text-yellow-800";
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[60] px-4 text-black" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-3xl max-h-[calc(100%-100px)] flex flex-col overflow-hidden" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            ...
            <aside className="space-y-5">
              <div className="rounded-xl border border-black/5 p-4">
                ...
                <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full font-semibold text-sm ${getLandTypeColor(deed.landType)}`}>
                  <span>{getLandTypeIcon(deed.landType)}</span>
                  <span className="capitalize">{deed.landType ?? "Unknown"}</span>
                </div>
              </div>
              ...
              <div className="rounded-xl border border-black/5 p-4">
                ...
                <div className="relative group mt-3 h-28 w-full rounded-lg border border-black/5 overflow-hidden">
                  <MapPreview points={deed.location} />
                  <button
                    onClick={() => setIsMapOpen(true)}
                    className="absolute top-2 right-2 bg-white p-1 rounded-full shadow opacity-0 group-hover:opacity-100 transition flex items-center justify-center z-50"
                  >
                    <FaExpand size={18} className="text-green-700" />
                  </button>
                </div>
              </div>
