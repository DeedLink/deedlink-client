...
interface DeedTableProps {
  deeds: RequestRegisteringDeed[];
  activeTab: "Pending" | "Approved" | "Minted" | "Rejected";
  onView: (deed: RequestRegisteringDeed) => void;
}
...
const DeedTable = ({ deeds, activeTab, onView }: DeedTableProps) => {
  ...
  const filteredDeeds = useMemo(() => {
    return deeds
      .filter((deed) => deed.status === activeTab)
      .filter(
        (deed) =>
          deed.id.toLowerCase().includes(search.toLowerCase()) ||
          deed.owner.toLowerCase().includes(search.toLowerCase()) ||
          deed.address.toLowerCase().includes(search.toLowerCase())
      );
        ...
        <div className="hidden md:block overflow-x-auto">
            ...
            <tbody>
                ...
                paginatedDeeds.map((deed) => (
                    ...
                    >
                    <td className="px-4 py-3 font-medium">{deed.id}</td>
                    <td className="px-4 py-3">{deed.owner}</td>
                    <td className="px-4 py-3">{deed.address}</td>
        ...
        <div className="md:hidden space-y-4">
            ...
            paginatedDeeds.map((deed) => (
                ...
                <div className="flex justify-between items-center mb-2">
                    <h3 className="font-bold text-green-800">{deed.owner}</h3>
                    <span
                    className={`px-3 py-1 rounded-full text-xs font-semibold ${
                        deed.status === "Pending"
                        ? "bg-yellow-100 text-yellow-700"
                ...
                <p className="text-sm text-gray-600">
                    <span className="font-semibold">Deed ID:</span> {deed.id}
                </p>
                <p className="text-sm text-gray-600">
                    <span className="font-semibold">Address:</span> {deed.address}
                </p>
                <div className="mt-3">
                    <button
                    onClick={() => onView(deed)}
