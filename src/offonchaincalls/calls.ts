...
import { registerDeed, requestValuation, updateTokenId, uploadMetadata } from "../api/api"
...
export const reg_mintNFT = async (to: string, data: any) => {
    ...
    const valuate = await requestValuation(reg.data._id, data.propertyValue);
    console.log("valuate: ", valuate);
